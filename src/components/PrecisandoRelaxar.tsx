...
import { useSurpresas } from "../data/surpresas";
    ...
    <div className="text-center my-10">
      {etapa === "video" && mostrar && (
        <div ref={videoRef} className="mt-8">
          <h2 className="text-xl font-semibold mb-4">{t('precisandoRelaxar.titulo_video')}</h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-sm sm:max-w-md md:max-w-lg rounded-2xl border-2 border-accent/30 dark:border-accent/50 p-6 dark:bg-gray-800 shadow-md shadow-accent/20 text-center">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-secondary dark:text-secondary/80 mb-4">
                {t('precisandoRelaxar.titulo_frase')} <strong>{contador}</strong> 
              </h2>
            </div>
      ...
      {etapa === "musica" && mostrar && (
        <div className="mt-8">
          <h2 className="text-xl font-semibold mb-4">{t('precisandoRelaxar.titulo_musica')}</h2>
          <button
            onClick={tocarMusica}
            className="px-6 py-2 bg-accent text-white rounded-xl hover:bg-accent/80 transition"
          >
            {t('precisandoRelaxar.botao_musica')}
          </button>
